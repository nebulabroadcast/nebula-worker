...
from nxtools import FileObject, format_time

from nebula.db import DB
from nebula.helpers import bin_refresh
from nebula.log import log
from nebula.objects import Bin, Event, Item
from nebula.plugins.common import get_plugin_path
...
class SolverPlugin:
    ...
    def block_split(self, tc):
        ...
        log.info(f"Splitting {self.event} at {format_time(tc)}")
        log.info(
    ...
    def main(self, debug=False, counter=0):
        log.info("Solving {}".format(self.placeholder))
        message = "Solver returned no items. Keeping placeholder."
        try:
            for new_item in self.solve():
                self.new_items.append(new_item)
                if debug:
                    log.debug("Appending {}".format(new_item.asset))
        except Exception:
            message = log.traceback("Error occured during solving")
    ...
    for f in [
        ...
        if f.exists:
            ...
            try:
                ...
                log.traceback("Unable to load plugin {}".format(solver_name))
                return
    else:
        log.error("{} does not exist".format(f))
        return

    if "Plugin" not in dir(py_mod):
        log.error("No plugin class found in {}".format(f))
        return
